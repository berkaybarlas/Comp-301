    ...
    (lambda (cont val)
      (cases continuation cont
        ...
        (map2-cont (procexp saved-env saved-cont)
                   (if (expval->null? val)
                           (emptylist-val)
                       (apply-cont (map3-cont procexp val saved-env saved-cont) val)        
                       )
                   )
        ...
        (map3-cont (procexp saved-val saved-env saved-cont)
                   (let ((val1 (expval->car saved-val)) (val2 (expval->cdr saved-val)) (proc1 (expval->proc procexp)) )
                     (if (expval->null? val2)
                         ...
                         ;(apply-cont (map3-cont procexp val2 saved-env saved-cont) val2)
                         (apply-procedure/k proc1 val1 (map3-cont procexp val2 saved-env (list2-cont val1 saved-env saved-cont)))
