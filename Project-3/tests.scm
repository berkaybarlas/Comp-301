  ...
  (define test-list
    '(
      ...
      (nested-procs2 "let f = proc(x) proc (y) -(x, y) in ((f -(10, 5)) 6)"
       ...
       (y-combinator-1 "let fix = proc (f) let d = proc (x) proc (z) ((f (x x)) z) in proc (n) ((f (d d)) n) in let t4m = proc (f) proc(x) if zero?(x) then 0 else -((f -(x, 1)), -4) in let times4 = (fix t4m) in (times4 3)" 12)
...
;     (even-odd-via-set-1 "
;mlet x = newref(0) in letrec even(d) = if zero?(deref(x)) 
;                   then 1
...
;                   then 0
;                   else let d = setref(x, -(deref(x), 1))
;                        in (even d) in let d = setref(x,13) in (odd -100)" 1)
;
; (even-odd-via-set-1 "
;mlet x = newref(0) in letrec even(d) = if zero?(deref(x)) 
;                   then 1
...
;   in let d = setref(x,13) in (odd -100)" 1)
 ...
 (show-allocation-1 "
let x = newref(22) in let f = proc (z) let zz = newref(-(z, deref(x))) in deref(zz)
   in -((f 66), (f 55))"
   11)
...
let x = newref(newref(0)) in begin 
 ...
 (array-detailed-test-0 "let a = newarray(2, -99) in read-array(a, 1)" -99)

      (array-detailed-test-1 "mlet a = newarray(2, -99)
                         p = proc (x)
      ...
      (array-detailed-test-2 "mlet a = newarray(3, 5)
                         p = proc (x)
                                  let v = read-array(x, 1)
                                  in update-array(x, 1, -(-2, v))
                         q = proc(x)
                                  mlet v1 = read-array(x, 1)
                                      v2 = read-array(x, 2)    
                                  in update-array(x, 1, -(v2, -(0, v1)))
                       in begin update-array(a, 1, -5); (p a); (q a); read-array(a, 1) end"
                      8)

      (array-detailed-test-3 "mlet a = newarray(2, -99)
                         p = proc (x)
                                  let v = read-array(x, 1)
